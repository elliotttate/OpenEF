...
void CG_RegisterWeapon( int weaponNum ) {
	...
	// if we couldn't find which weapon this is, warn and skip
	// (alien/NPC weapons like weapon_forge_proj may not have item entries).
	// Don't set registered=qtrue so callers know this weapon has no model data.
	if ( !item->classname ) {
		cgi_Printf( va("WARNING: No item entry for weapon %s (weapon %d) - skipping registration\n",
			weaponData[weaponNum].classname, weaponNum) );
		weaponInfo->registered = qtrue;  // prevent re-entry but data is zeroed
		return;
	}
	...
	{
		cgi_Printf( va("WARNING: Missing weapon model for %s\n", weaponData[weaponNum].classname) );
		return;
	}
