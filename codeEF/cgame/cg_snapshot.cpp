...
void CG_ProcessSnapshots( void ) {
	...
	if ( cg.snap->serverTime > cg.time ) {
		// In SP mode with ioEF, the server runs ahead before the client connects.
		// Sync cg.time forward instead of erroring.
		cg.time = cg.snap->serverTime;
	}
	if ( cg.nextSnap != NULL && cg.nextSnap->serverTime <= cg.time ) {
		// In SP mode with ioEF, timing can be off during initial frames.
		// Adjust time instead of erroring.
