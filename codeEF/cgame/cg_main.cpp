...
================
*/
extern "C" Q_EXPORT int QDECL vmMain( int command, int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7 ) {
	switch ( command ) {
	case CG_INIT:
		CG_Init( arg0 );
		return 0;
...
=================
*/
// When cgame is built as a separate DLL, CG_InitGiCompat provides
// stub gi.* functions. When built together with the game (combined DLL),
// gi is already populated by GetGameAPI so we don't need it.
#ifdef CGAME_DLL
extern void CG_InitGiCompat(void);
#endif
