...
Sets the defend target of an entity
=============
*/
...
static void Q3_SetHidingGoal( int entID, const char *name )
{
	...
	if (goal == NULL)
	{
		Q3_DebugPrint( WL_WARNING, "Q3_SetHidingGoal: can't find HidingGoal target: '%s'\n", name );
		return;
	}
...
static void Q3_Set( int taskID, int entID, const char *type_name, const char *data )
{
	...
	case SET_HIDING:
		Q3_SetHidingGoal( entID, (char *) data );
		break;
	...
	case SET_DEFEND_TARGET:
		Q3_SetDefendTarget( entID, (char *) data );
		break;
...
============
*/
static float Q3_GetFaceHoldSeconds( gentity_t *ent, int expression )
{
	float endTime = 0.0f;
	...
	{
	case SET_FACEAUX:
		if ( ent->client->facial_aux >= 0.0f )
		{
			return 0.0f;
		}
		endTime = -ent->client->facial_aux;
		break;
	case SET_FACEBLINK:
		if ( ent->client->facial_blink >= 0.0f )
		{
			return 0.0f;
		}
		endTime = -ent->client->facial_blink;
		break;
	case SET_FACEBLINKFROWN:
		if ( ent->client->facial_blink >= 0.0f || ent->client->facial_frown >= 0.0f )
		{
			return 0.0f;
		}
		endTime = -ent->client->facial_blink;
		if ( -ent->client->facial_frown > endTime )
		{
			endTime = -ent->client->facial_frown;
		}
		break;
	case SET_FACEFROWN:
		if ( ent->client->facial_frown >= 0.0f )
		{
			return 0.0f;
		}
		endTime = -ent->client->facial_frown;
	...
	return ( endTime - level.time ) * 0.001f;
}
...
//		memset(&client->sess,0,sizeof(client->sess));
	...
	case SET_FACENORMAL:		//## %f="0.0" # Set face to Normal expression for number of seconds
		if ( ent->client == NULL )
		{
			Q3_DebugPrint( WL_WARNING, "Q3_GetFloat: SET_FACE___, %s not a client\n", ent->targetname );
			return false;
		}
		*value = Q3_GetFaceHoldSeconds( ent, toGet );
		break;
	...
	case SET_ANIM_HOLDTIME_BOTH://## %d="0" # Hold lower and upper anims for number of milliseconds
		if ( ent->client == NULL )
		{
			Q3_DebugPrint( WL_WARNING, "Q3_GetFloat: SET_ANIM_HOLDTIME_BOTH, %s not a client\n", ent->targetname );
			return false;
		}
		*value = ( ent->client->ps.legsAnimTimer > ent->client->ps.torsoAnimTimer ) ? ent->client->ps.legsAnimTimer : ent->client->ps.torsoAnimTimer;
		break;
	...
	case SET_INTERFACE://## %t="BOOL_TYPES" # Player interface on/off
		*value = gi.Cvar_VariableIntegerValue( "cg_drawStatus" ) ? 1.0f : 0.0f;
		break;
	...
	case SET_VIDEO_FADE_IN://## %t="BOOL_TYPES" # Makes video playback fade in
		*value = gi.Cvar_VariableIntegerValue( "cl_VidFadeUp" ) ? 1.0f : 0.0f;
		break;
	case SET_VIDEO_FADE_OUT://## %t="BOOL_TYPES" # Makes video playback fade out
		*value = gi.Cvar_VariableIntegerValue( "cl_VidFadeDown" ) ? 1.0f : 0.0f;
		break;
...
static int Q3_GetString( int entID, int type, const char *name, char **value )
{
	...
	case SET_NAVGOAL://## %s="NULL" # *Move to this navgoal then continue script
		if ( ent->NPC == NULL )
		{
			Q3_DebugPrint( WL_WARNING, "Q3_GetString: SET_NAVGOAL, %s not an NPC\n", ent->targetname );
			return false;
		}
		else if ( ent->NPC->goalEntity != NULL && VALIDSTRING( ent->NPC->goalEntity->targetname ) )
		{
	...
	case SET_VIEWTARGET://## %s="NULL" # Set angles toward ent by targetname
		Q3_DebugPrint( WL_WARNING, "Q3_GetString: SET_VIEWTARGET is write-only\n" );
		return false;
		break;
	...
	case SET_COPY_ORIGIN://## %s="targetname"  # Copy the origin of the ent with targetname to your origin
		Q3_DebugPrint( WL_WARNING, "Q3_GetString: SET_COPY_ORIGIN is write-only\n" );
		return false;
		break;
	case SET_DEFEND_TARGET://## %s="targetname"  # This NPC will attack the target NPC's enemies
		if ( ent->NPC == NULL )
		{
			Q3_DebugPrint( WL_WARNING, "Q3_GetString: SET_DEFEND_TARGET, %s not an NPC\n", ent->targetname );
			return false;
		}
		else if ( ent->NPC->defendEnt != NULL )
		{
			*value = ent->NPC->defendEnt->targetname;
		}
		else return false;
		break;
	...
	case SET_LOCKYAW://## %s="off"  # Lock legs to a certain yaw angle (or "off" or "auto" uses current)
		if ( ent->client == NULL )
		{
			Q3_DebugPrint( WL_WARNING, "Q3_GetString: SET_LOCKYAW, %s not a client\n", ent->targetname );
			return false;
		}
		if ( !( ent->client->renderInfo.renderFlags & RF_LOCKEDANGLE ) )
		{
			*value = "off";
		}
		else
		{
			*value = va( "%g", ent->client->renderInfo.lockYaw );
		}
		break;
