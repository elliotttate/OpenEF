...
void OBJ_ParseObjectives(char *buffer)
{
	...
	for (int _oi = 0; _oi < MAX_OBJECTIVES; _oi++) {
		mission_objective_text[_oi] = (char *)defstr;
	}

	for (int _oi = 0; _oi < MAX_TOUR_OBJECTIVES; _oi++) {
		tour_objective_text[_oi] = (char *)defstr;
	}
...
void OBJ_ParseTactical(char *buffer)
{
	...
	for (int _ti = 0; _ti < MAX_TACTICAL_TXT; _ti++) {
		tactical_text[_ti] = (char *)defstr;
	}
